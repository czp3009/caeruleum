...
inline fun <reified T> HttpClient.create(baseUrl: String? = null) =
    createDynamicProxyToHttpClient(this, T::class, baseUrl) as T

@PublishedApi
internal fun createDynamicProxyToHttpClient(httpClient: HttpClient, kClass: KClass<*>, baseUrl: String?): Any {
    val jClass = kClass.java
    //check interface
    require(jClass.isInterface) { "API declaration must be interface" }
    require(jClass.interfaces.isEmpty()) { "API interface must not extend other interfaces" }
    //create proxy
    return Proxy.newProxyInstance(
        jClass.classLoader,
        arrayOf(jClass),
        CaeruleumProxy(httpClient, kClass, jClass, baseUrl)
    )
}
